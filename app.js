...
function loadAdmin(){
 ...
 complaints.forEach((c,i)=>{
  adminList.innerHTML+=`<div class="box"><h3>${c.title}</h3><p>${c.desc}</p>
   <select id="s${i}"><option ${c.status=="Pending"?"selected":""}>Pending</option>
   <option ${c.status=="Resolved"?"selected":""}>Resolved</option></select>
...
function deleteComplaint(i){complaints.splice(i,1);localStorage.setItem("complaints",JSON.stringify(complaints));loadMy();loadTeacher();loadAdmin();loadChart();}
